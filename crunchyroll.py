...
def timestamp_to_datetime(ts):
    return (datetime.datetime.strptime(ts[:-7], '%Y-%m-%dT%H:%M:%S') + datetime.timedelta(hours=int(ts[-5:-3]),
                                                                                          minutes=int(ts[-2:])) * -int(
        ts[-6:-5] + '1')).replace(tzinfo=tz.tzutc()).astimezone(tz.tzlocal())
...
# TODO: Currently the session is dropped entirely if the authentication fails.
# TODO: We want to cache and re-use it on the next attempt!
...
def show_queue(args=None):
    ...
    if queue is None or "update" in args:
        update_queue()
        if queue is None:
            return
